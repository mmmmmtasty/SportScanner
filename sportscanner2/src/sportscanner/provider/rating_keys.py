...
GUID_PREFIX = "tv.plex.agents.custom.sportscanner.metadata"
...
def make_show_guid(competition_id: str) -> str:
    return f"{GUID_PREFIX}://show/{make_show_rating_key(competition_id)}"


def make_season_guid(competition_id: str, season_number: int) -> str:
    return f"{GUID_PREFIX}://season/{make_season_rating_key(competition_id, season_number)}"


def make_episode_guid(segment_id: str) -> str:
...
def parse_guid(guid: str) -> tuple[str, tuple[str, ...]]:
